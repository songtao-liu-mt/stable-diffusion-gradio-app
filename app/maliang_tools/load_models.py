...
assets_dir = os.path.join(__dir__, '../mt_images')
...
vae = AutoencoderKL.from_pretrained(
    os.path.join(models_dir, 'diffusers/runwayml/stable-diffusion-v1-5'),
...
tokenizer = CLIPTokenizer.from_pretrained(
    ...
    os.path.join(models_dir, 'diffusers/runwayml/stable-diffusion-v1-5/tokenizer'),
    )
text_encoder = CLIPTextModel.from_pretrained(
    # 'openai/clip-vit-large-patch14',
    os.path.join(models_dir, 'diffusers/runwayml/stable-diffusion-v1-5/text_encoder'),
    # revision="fp16", 
...
#     os.path.join(models_dir, 'models/diffusers/runwayml/stable-diffusion-inpainting'),
#     # 'IDEA-CCNL/Taiyi-Stable-Diffusion-1B-Chinese-EN-v0.1',
#     subfolder='unet',
#     revision="fp32", 
#     torch_dtype=torch.float32,
...
real_esr = RealESGANWrapper(model_path=os.path.join(models_dir, 'realesrgan/RealESRGAN_x4plus.pth'), device=device)
translator_zh2en = TranslatorWrapper(model_path=os.path.join(models_dir, 'opus_mt'))
image2text_pipeline = BlipWrapper(model_path = os.path.join(models_dir, 'blip/model_base_capfilt_large.pth'), device=device)
adain_trasfer = AdaINWrapper(vgg_path=os.path.join(models_dir, 'adain/vgg_normalised.pth'), 
                             decoder_path=os.path.join(models_dir, 'adain/decoder.pth'),
                            device=device
                            )
clipseg = CLIPSegWrapper(model_path = os.path.join(models_dir, 'clipseg/rd64-uni.pth'), device=device)


logo_image_pil = Image.open(os.path.join(assets_dir, 'maliang_mt_logo.png')).convert('RGB')
forbidden_pil = Image.open(os.path.join(assets_dir, 'forbidden.jpg')).resize((1000, 1000)).convert('RGB')
gc.collect()
